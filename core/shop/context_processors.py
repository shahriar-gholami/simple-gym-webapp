...
def base_template_context(request):
	...
	if second_segment != 'viracoders' and third_segment != None and 'admin' not in path_segments and second_segment!='blog' and not path_segments[-2].startswith('09') and third_segment != 'orders':
		...
		if isinstance(request.user, AnonymousUser):
			...
			cart_url = f'shop:cart_view'
			owner = Owner.objects.filter(phone_number = request.user.phone_number, store=store).first()
			if owner != None:
				account_url = f'shop:owner_dashboard'
			else:
				account_url = f'shop:customer_dashboard'
			return {
					'user_info': user_info,
					'current_path': current_path,
					'second_segment': second_segment,
